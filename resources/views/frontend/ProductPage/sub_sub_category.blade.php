...
@section('mainpage')
@section('title')
{{ session()->get('language')=='bangla'? $sub_sub_category_one->subsubcategory_name_bn : $sub_sub_category_one->subsubcategory_name_en }}
@endsection
<!-- <style>
  .sidebar {
    margin-top: 50px;
...
<div class="content_one" style="min-height: 1000px; padding-top: 80px;">
  <div class="container">
    <div style="display: flex;"> <a href="{{ url('/')}}" style="text-decoration: none;">@if (session()->get('language')=='bangla')
      ...
      <h1> /</h1>
      <a href="{{ route('productPage.category', [ 'mainCategory_id'=>$sub_sub_category_one->category->id, 'mainCategory_name'=>$sub_sub_category_one->category->category_name_en] )}}" style="text-decoration: none;">
        <h1>{{ session()->get('language')=='bangla'?$sub_sub_category_one->category->category_name_bn : $sub_sub_category_one->category->category_name_en }} </h1>
      </a>
      <a href="{{ route('productPage.subcategory', ['subCategory_id'=> $sub_sub_category_one->subcategory->id, 'subCategory_name'=> $sub_sub_category_one->subcategory->subcategory_name_en] )}}" style="text-decoration: none;">
        <h1>/ {{ session()->get('language')=='bangla'? $sub_sub_category_one->subcategory->subcategory_name_bn : $sub_sub_category_one->subcategory->subcategory_name_en }} </h1>
      </a>
      <h1>/ {{ session()->get('language')=='bangla'? $sub_sub_category_one->subsubcategory_name_bn : $sub_sub_category_one->subsubcategory_name_en }} </h1>
    </div>
...
<script type="text/javascript">
  function ShowSlider() {
    $.ajax({
      type: 'GET',
      url: '/slider/subsubcategory/show/{{ $sub_sub_category_one->id }}',
      dataType: 'json',
...
<script>
  var subsubcategoryElement = document.getElementById("ssubc_{{ session()->get('language')=='bangla'? $sub_sub_category_one->subsubcategory_name_bn : $sub_sub_category_one->subsubcategory_name_en }}");
  subsubcategoryElement.style.backgroundColor = "crimson";
  var x = document.getElementById("sub_category_{{$sub_sub_category_one->category->id}}");
