...
<div class="content_one" style="min-height: 1000px; padding-top: 80px;">
  <div class="container">
    <div style="display: flex;"> <a href="{{ url('/')}}" style="text-decoration: none; ">@if (session()->get('language')=='bangla')
      ...
      <h1>/ {{ session()->get('language')=='bangla'?$main_category_one->category_name_bn : $main_category_one->category_name_en }}
      </h1>
    </div>
    <div class="row">
      @foreach ($sub_category_one as $sub1)
        <a href="{{ route('productPage.subcategory', ['subCategory_id'=>$sub1->id, 'subCategory_name'=>$sub1->subcategory_name_en] )}}" type="button" class="btn btn-light btn-sm mx-1" style="width: fit-content;">{{ session()->get('language')=='bangla'? $sub1->subcategory_name_bn : $sub1->subcategory_name_en }}</a>
      @endforeach
    </div>
    <!-- <div class="row">
      @foreach ($author_all as $author)
        <a href="{{ route('productPage.author', ['author_id'=>$author->id, 'author_name'=>$author->author_name_en] )}}" type="button" class="btn btn-light btn-sm mx-1" style="width: fit-content;">{{ session()->get('language')=='bangla'? $author->author_name_bn : $author->author_name_en }}</a>
      @endforeach
    </div> -->
...
@push('custom-scripts')
<script>
  var categoryElement = document.getElementById("cate_{{ session()->get('language')=='bangla'?$main_category_one->category_name_bn : $main_category_one->category_name_en }}");
